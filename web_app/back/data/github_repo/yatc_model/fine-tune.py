...
import timm
parent_dir = os.path.dirname(os.path.realpath("/kaggle/input/yatc_model_modifie/YaTCKaggle/util"))
#module_path =  os.path.dirname(os.path.realpath("/kaggle/input/netMamba/NetMambaKaggle/mamba-1p1p1/mamba_ssm"))
...
#20
    ...
    parser.add_argument('--log_dir', default='logs_dir',
...
def main(args):
    ...
    dataset_train = build_dataset(is_train=args.is_train , args=args)
    dataset_val = build_dataset(is_train=args.is_eval, args=args)
    ...
    if args.finetune and not args.eval:
        ...
        # Check if checkpoint is a dictionary (standard format) or direct model (your saved format)
        if isinstance(checkpoint, dict):
            ...
            # Direct model format (your saved format)
            print("Loading directly saved model")
    ...
    torch.save(model, "output_dir/yatc/finetuned_yatc_{}.pth".format(args.data_path.split("/")[-1]))
