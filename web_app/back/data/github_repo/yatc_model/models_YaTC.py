...
class TrafficTransformer1(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        x = x.mean(dim=1)
        print("Shape before LayerNorm:", x.shape)  # Debug

        #outcome = self.fc_norm(x)
        print("Shape after LayerNorm:", outcome.shape)  # Debug
...
class TrafficTransformer(timm.models.vision_transformer.VisionTransformer):
    def __init__(self, **kwargs):
        ...
        # Add a classifier head if needed
    ...
    def forward_packet_features(self, x, i):
        ...
        return self.fc_norm(x)  # Fixed: added return statement
    ...
    def forward_features(self, x):
        ...
        for i in range(5):
            ...
            packet_x = packet_x.reshape(B, C, -1, 40)
            packet_x = self.forward_packet_features(packet_x, i)
            feature_vectors.append(packet_x)
            
        # Concatenate along dimension 1 (sequence length)
        x = torch.cat(feature_vectors, dim=1)
        print(f"Shape after collecting features: {x.shape}")
        
        # Process through blocks again if needed
        # for blk in self.blocks:
        #    x = blk(x)
        
        # Global pooling - average over the sequence dimension but preserve embed_dim
        x = x.mean(dim=1)  # Result should be [B, embed_dim]
    ...
    def forward(self, x):
        ...
        # Apply classifier head to get the class predictions
        x = self.head(x)
